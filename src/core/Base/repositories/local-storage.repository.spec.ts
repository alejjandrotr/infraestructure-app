...
describe('LocalStorageRepository', () => {
  const repository = new LocalStorageRepository<Sala>(config);
  ...
  test('should add a new item', async () => {
    ...
    expect(
      JSON.parse(localStorage.getItem(config.path) || '[]')
    ).toContainEqual(newSala);
  });
  ...
  test('should edit an existing item', async () => {
    ...
    expect(
      JSON.parse(localStorage.getItem(config.path) || '[]')
    ).toContainEqual({ id: '3', ...updatedSala });
  });
